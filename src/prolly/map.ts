...
import {deepEqual, ImmutableJSONValue} from '../json';
import {arrayCompare} from './array-compare';
import {Leaf} from './leaf';
import {PeekIterator} from './peek-iterator';
import {stringCompare} from './string-compare';

export type Entry = [key: string, value: ImmutableJSONValue];
...
class ProllyMap {
  ...
  // for now.
  private readonly _pending: Map<string, ImmutableJSONValue | DeleteSentinel>;

  constructor(
    base: Leaf | undefined,
    pending: Map<string, ImmutableJSONValue | DeleteSentinel>,
  ) {
    this._base = base;
  ...
  private _baseGet(key: string): ImmutableJSONValue | undefined {
    if (this._base === undefined) {
      return undefined;
    }
  ...
  put(key: string, val: ImmutableJSONValue): void {
...
type DeletableEntry = readonly [
  key: string,
  val: ImmutableJSONValue | DeleteSentinel,
];
...
class Iter implements IterableIterator<Entry> {
  ...
  constructor(
    ...
    const p: [string, ImmutableJSONValue | DeleteSentinel][] = [...pending];
